...
- [ ] **Concluir o setup do Claude Code:** criar no repo os arquivos de arranque entregues (`CLAUDE.md` raiz, `.claude/settings.json`, `.claude/commands/apply-spec.md`, `.claude/commands/wrap.md`), renomear `meta/CLAUDE.md`→`meta/CEREBRO.md`, e **apontar as Instruções do Projeto (painel) de `CLAUDE.md` para `CEREBRO.md`**. Rodar uma sessão de teste do Code (ex.: `/wrap`) para validar permissões e build.
...
- [ ] **Atualizar o HUB.md** — corrigir o status relâmpago do ASU (cita v0.4.0 → está em 0.6.0). É um único arquivo na raiz comum dos 3 projetos (não há cópias a sincronizar); entregar a versão nova substitui a anterior.
...
## 💬 Última Sessão
**2026-06-21 — adoção do modo Claude Code; CLAUDE.md→CEREBRO.md (sem código).** Atualização do KCM ("update-code-mode") integrada. **Entregue:** `meta/CEREBRO.md` (nosso antigo CLAUDE.md renomeado + nova seção «Desenvolvimento no Claude Code», com a seção HUB preservada); arquivos de arranque na raiz do repo (`CLAUDE.md` ponteiro curto, `.claude/settings.json`, `.claude/commands/apply-spec.md` e `wrap.md`), adaptados ao ASU (build = pytest/self-test/ruff/black; nomes de doc do nosso nicho, não os do template). DEC-021 registra a decisão; CONTEXT atualizado (rename + workflow). **Decisão:** o CEREBRO foi entregue como arquivo INTEIRO pelo chat (reescrita fundamental = raia do chat; o método novo reserva specs para deltas pequenos, e o Code ainda não estava configurado). **Achados:** os `.txt` citados (`260621-Sugestões...`, `260619-ideias.txt`) NÃO vieram no pacote — pendência de upload; o template de arranque do KCM usa nomes de doc do nicho Design (`DECISOES`/`REVISOES`) que não batem com o nosso nicho Dev (`DECISIONS`) — corrigido na adaptação e registrado como feedback ao Kit. **Pendente:** criar os arquivos no repo + apontar as Instruções do Projeto para CEREBRO; subir os `.txt` se quiser que eu os processe; herdados: HUB status relâmpago, README/GUIA. Sessões anteriores:
