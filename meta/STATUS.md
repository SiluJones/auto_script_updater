...
## Versão Atual
**[0.7.0]** — 2026-06-23 — **F2 increment "Acesso rápido a projetos" completo.** Quatro itens implementados: (1) pastas-raiz recentes (máx. 8, case-insensitive) + fixadas, menu "Recentes ▾" + botão 📌, persistidos em QSettings; (2) args de lançamento `--root`/`--instruction-dir`/`--instruction` via argparse; (3) resolução pasta→instrução (só o topo; 1 yaml = pré-preenche, 2+ = aponta seletor); (4) botão "Criar atalho .bat…" + módulo puro `src/gui/launcher.py` (python do venv direto, sem activate — DEC-022). **112 testes verdes; ruff/black limpos.**

> **Anterior:** [0.6.0-alpha] — 2026-06-15 — conveniências de backup + paridade de sandbox na GUI (93 testes).
...
## 🔧 Em Progresso
- **F2 (GUI) — itens estruturais restantes** (próximo increment): validação VISUAL no Windows real (o usuário rodou `python -m src.gui` mas ainda não relatou a experiência da janela, incluindo os novos botões 📌/Recentes/.bat); highlight de sintaxe no diff; barra de progresso; tema claro/escuro; seleção de timestamps antigos no Desfazer; threads se surgir caso de lentidão.
...
- [x] **▶ Spec `meta/specs/F2-acesso-rapido.md` implementada (2026-06-23):** recentes/fixadas + args de lançamento + gerador de .bat + resolução pasta→instrução. 112 testes verdes; ruff/black limpos.
...
## 💬 Última Sessão
**2026-06-23 — implementação completa da spec F2-acesso-rapido.md no Claude Code.** WI-1 (recentes/fixadas: `_push_recent_root`, `_toggle_pin_root`, menu "Recentes ▾", botão 📌), WI-2 (args de lançamento via argparse; `_apply_instruction_dir` com resolução D3), WI-3 (módulo puro `src/gui/launcher.py`: `resolve_instruction_in_dir` + `build_launcher_bat`), WI-4 (`_create_launcher_bat` + `btn_bat`). Dois commits separados por WI (WI-3 solo; WI-1+2+4 juntos em `main_window.py`). 112 testes verdes (+19 vs. 93); ruff limpo; black limpo. Versão bumped para 0.7.0. Docs atualizados (STATUS, CHANGELOG, ROADMAP). **Pendente:** validação visual no Windows (o usuário precisa rodar `python -m src.gui` e testar os novos botões e o .bat gerado).
