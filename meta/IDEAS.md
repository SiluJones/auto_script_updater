...
### 2026-07-03 — Mensagem de erro do validador mais didática (schema `minLength`) — CONCLUÍDA (spec0003, 0.8.4, commit `e6b5aff`)
Um usuário recebeu `$.files[5].modifications[0].location.after: '' should be non-empty` ao aplicar uma instrução gerada por IA (o `after` do `replace_context_block` veio vazio — provável causa: o bloco-alvo ia até o FIM do arquivo, sem linha "depois" para ancorar). O ASU acertou (barrou antes de tocar arquivo, com o caminho preciso), mas a mensagem é o texto CRU do jsonschema, em inglês, sem porquê nem conserto. **Ideia:** em `instruction_validator._format_error`, traduzir/enriquecer os erros de schema mais comuns — em especial `minLength`/`non-empty` em `location.before`/`after` — para algo como *"'after' está vazio: use uma linha ASCII estável logo abaixo do bloco; se o bloco vai até o fim do arquivo, `replace_context_block` não serve — prefira `replace_line_pattern`/`insert_before_pattern` ancorando numa linha existente, ou a estratégia própria do tipo (`replace_section`/`replace_function`)."* Também vale uma nota no `INSTRUCTION_GUIDE` (seção do `replace_context_block`): **não usar quando o bloco toca a borda do arquivo**. Baixo risco (só formatação de mensagem + doc); candidato a uma spec DOC+FIX leve. Conecta com §4.7 (âncoras ASCII) e com a DEC-026 (mensagens acionáveis).

### 2026-07-03 — Emissões adicionais do canal de warnings — follow-up (base pronta: DEC-028)
O canal de warnings foi entregue (DEC-028, specs 0001+0002) com UM piloto (`create_file` sobrescrevendo). A base — retorno opcional em tupla, `warnings`, `has_warnings`, 🟡/⚠ na GUI — já existe; emitir um aviso novo agora é só a estratégia retornar `(texto, avisos)`, sem novo encanamento. Emissões que valem adicionar, cada uma com seu teste, sob demanda: (1) âncora casada por fuzzy de whitespace nas estratégias de texto (hoje o fuzzy só vive no caminho de ERRO/dica — levar ao caminho de SUCESSO como aviso); (2) `occurrence` ausente com >1 match resolvido por default=1 (avisar que dependeu de ordem); (3) âncora adjacente a trecho Unicode frágil (§4.7). Não fazer em massa — entra quando o uso pedir.

### 2026-06-30 — Cobertura da dica "já aplicado?" além de texto/pattern — EM ABERTO (refino futuro da DEC-026)
A DEC-026 entregou as dicas `_substring_hint` + `_already_applied_hint` (agregadas em `_anchor_hints`) para o erro de âncora, **sem ledger** (detecção em memória só no caminho de erro). Cobre bem `type: text`/`pattern`/`replace_context_block`. **Deixado conscientemente em aberto** (não dar a DEC por 100% concluída): (1) **estratégias estruturais** — `set_json_path`/`append_json_array`/`delete_json_path` não passam por `_anchor_hints` (o "já aplicado" delas seria "a chave/valor-alvo já existe com esse valor", lógica diferente, no `json_strategy`); (2) **`replace_function`/`replace_method`/`replace_class`** (libcst, `python_strategy`) — a falha "símbolo não encontrado" não recebe a dica de "já aplicado" hoje. (3) **Limite conhecido do `_substring_hint` em `pattern`:** ele compara o **texto BRUTO do regex** (sem decodificar escapes), então um `pattern` sem metacaracteres já teria casado via regex antes de chegar ao caminho de erro — a dica de substring é muito mais útil para âncoras LITERAIS (`before`/`after`) do que para `pattern`. Por isso o teste de regressão usa um cenário com barra invertida literal (estilo caminho Windows) — ver `test_substring_hint_em_insert_after`. **Próximo passo, sob demanda:** se o uso real mostrar fricção em JSON/Python, estender o conceito de "já aplicado" a cada família com a lógica própria dela (hash estrutural por estratégia?), em vez de forçar a comparação textual onde não cabe. **Pesquisar** antes: como harnesses lidam com "idempotência" em edição estrutural (não só textual). Sem urgência; registrado para não reabrir do zero.
...
### 2026-07-03 — 2ª atualização do KCM integrada (conjunto completo + painel): o que adotamos e o que refinar
O KCM trouxe o conjunto completo de `*__template-update.md` + o `instrucoes-dev__template-update.txt` (template do painel). Ao cruzar com o nosso CEREBRO (já muito alinhado das sessões anteriores), o delta real foi pequeno — bom sinal de convergência. Adotado (DEC-027): **config-no-Code** (`/effort`/`ultrathink`, sem toggle de pensamento no Code); **convenção de spec do KCM** `AAMMDD-specNNNN-desc.md` + `AAMMDD-asuNNNN.yaml` (legado `F<n>-slug` mantido); **`HISTORICO.md`→`HISTORY.md`**; **painel atualizado** (releitura do mount ao sinalizar upload, nome de download simples, config no Code, `.gitignore`/README por previsão). Pontos de feedback ao kit:
- **Templates de doc não mudaram de estrutura** (STATUS/CONTEXT/DECISIONS/IDEAS/ROADMAP/CHANGELOG/GLOSSARY/HISTORY) — acertado: nossos docs são instâncias mais ricas, nada a regenerar. Continua sendo a decisão certa não forçar regeneração.
- **Seção de HUB ainda no molde "grupo de conteúdo"** (lore/mecânica/visual/som; "cópia idêntica em cada projeto, o usuário sincroniza"). Não cabe num HUB de toolchain/infra gerado pela conversa do KCM em modo só-HUB (o nosso). Preservamos a versão rica do projeto. **Reincidência** do feedback de 06-19 (Refinar 3) e 06-30 — o kit poderia oferecer variante "toolchain/infra" da seção de HUB.
- **Geração do template-update depende dos modos ligados** (feedback de 06-30 confirmado de novo na prática): a 1ª exportação saiu com a combinação errada de modos. Sugestão mantida: a página de geração sinalizar quais modos estão ativos ao exportar.
- **`HISTORICO`→`HISTORY`:** boa padronização (inglês, alinhado aos demais nomes). Só exige um `git mv` no repo de quem já tinha `HISTORICO`.

### 2026-06-30 — Template-update do KCM: geração depende dos MODOS ligados + seção «Recomendação de config» adotada
Ao integrar a atualização do KCM nesta sessão, dois pontos:
- **Geração sensível aos modos (achado de uso, não bug do kit):** o usuário gerou primeiro o `CEREBRO__template-update.md` com o **modo ASU ligado e o resto desligado** — saiu um template curto, com a seção «Saída via ASU» e SEM partes que este projeto usa. Regerou com os modos corretos → `CEREBRO__template-updates.md` (306 linhas, completo). Lição para o kit: a página de geração poderia **sinalizar quais modos estão ativos** no momento de exportar o template (ou avisar "você está exportando com modo X só"), porque o template resultante muda bastante e é fácil exportar com a combinação errada sem perceber.
- **«Recomendação de configuração (fim de sessão)» — ADOTADA:** o template-update novo traz essa seção (modelo/esforço/pensamento, recomendar pela tarefa, nunca afirmar a config atual). Era regra que este projeto já praticava (estava nas Instruções do Projeto) mas não como seção do CEREBRO. Puxada para o `meta/CEREBRO.md` nesta sessão, adaptada aos níveis que o usuário usa (Baixo→Máximo). Também documentada a **convenção de nome de specs** (`F<n>-<slug>.md`) que já praticávamos sem estar escrita.
- **Preservado (não regredir):** as seções «Projeto em grupo (HUB)» e «Desenvolvimento no Claude Code» do nosso CEREBRO são versões mais ricas (toolchain/infra, modo só-HUB) que as genéricas do template; mantidas. Reforça o feedback de 2026-06-19 (Refinar 3): o kit ainda gera a seção de HUB no molde de "grupo de conteúdo" (lore/visual/som), que não cabe num HUB de toolchain.

### 2026-06-30 — Observação do VectorForge: usar o ASU nos PRÓPRIOS docs do projeto (a levar ao KCM)
O VectorForge levantou (e o usuário pretende levar ao chat do KCM): seria viável o ASU editar os próprios meta-docs via `type: markdown` + `replace_section`? Análise dele, que endossamos: **híbrido é o certo.** `replace_section` funciona bem em arquivos com headings ÚNICOS — cobre `DECISIONS.md` (cada `## DEC-N — Título` é único) e boa parte do `CONTEXT.md`. MAS `STATUS.md`, `CHANGELOG.md` e `IDEAS.md` têm edições inerentemente **holísticas** (mover item resolvido do Backlog→Changelog, reclassificar uma ideia, a checagem do princípio 12 "nada único se perdeu") — exigem reler o arquivo inteiro, não uma substituição cirúrgica; patch nesses iria contra a própria regra de higiene do CEREBRO. Além disso, o **ganho de token do ASU é proporcional ao tamanho**: num `.html` de 86 KB compensa; num `CONTEXT.md` de 6 KB, regenerar inteiro já é barato e preserva a garantia holística. **Recomendação registrada:** ASU só para código (já é o caso); arquivo completo para docs rolantes; reavaliar quando `DECISIONS.md` se aproximar das ~700 linhas que ele mesmo prevê como gatilho de arquivamento — aí o cálculo muda. NB: isto NÃO conflita com o "modo só-HUB" (DEC-020), que trata de o ASU se auto-aplicar como mecanismo de ENTREGA; aqui é sobre a economia/ergonomia de patch em doc, decidida caso a caso. Conecta com a DEC-026 (limites de cobertura do patch em estruturas não-textuais).
