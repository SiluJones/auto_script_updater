...
### 2026-06-19 — Risco do ASU em código vs. segurança em .md narrativo — OBSERVAÇÃO VÁLIDA (vira ideia: validação de sintaxe pós-patch)
O usuário observou, com razão, que o ASU é seguro para atualizar documentos `.md` de narrativa/histórico (texto é texto; um patch malfeito gera no máximo prosa errada, fácil de ver no diff), mas tem um perigo real em CÓDIGO: um patch pode aplicar "com sucesso" (o localizador casou, a string foi escrita) e mesmo assim quebrar o arquivo — e o ASU NÃO compila nada, então esse tipo de erro não é sinalizado como seria por um compilador. A pesquisa confirma o diagnóstico (patches que passam no teste primário mas quebram em casos adversos; sintaxe ≠ semântica). Distinção importante: o ASU já valida implicitamente JSON (json.loads) e Python via `replace_function/method/class` (libcst parseia o `new_content`); o gap é o caminho `type: text` + `replace_context_block` num arquivo de código (.py/.js/.cs/…), que é só substituição de string, sem reparse do RESULTADO. RESPOSTA À DOR: ver a ideia do assistente «validação de sintaxe pós-aplicação (opt-in, por tipo)» — pega exatamente a categoria "quebrou a compilação", que é o medo do usuário. NÃO pega erro semântico (variável trocada, lógica invertida) — para esse, a rede continua sendo o diff + a §8 do guia (verificação pós-aplicação pela IA). Mantido como ideia ativa porque conecta a decisão de NÃO auto-aplicar o ASU sobre si (DEC-020) a uma melhoria concreta de produto.
...
### 2026-06-19 — Validação de sintaxe pós-aplicação (opt-in, por tipo de arquivo) — rede contra "patch aplicou mas quebrou o código"
Resposta direta ao medo do usuário (ver ideia dele 2026-06-19): após aplicar (ou em dry-run) uma modificação que produz código, reparsear o RESULTADO e, se não compilar, tratar como falha — disparando o rollback que já existe. Cobertura por tipo: **Python** → `ast.parse`/`py_compile` no conteúdo final (pega erro de sintaxe introduzido por um `replace_context_block` em `type: text` num `.py`); **JSON** → já validado de fato (o `json_strategy` carrega/regrava via `json`); demais linguagens → sem parser embutido no momento (poderia plugar tree-sitter no futuro — ver ideia tree-sitter — mas isso é semântico/estrutural, mais pesado). Desenho: flag `validate_syntax` em `settings` (default? avaliar — talvez ON para `.py` por baixo custo e alto valor, mantendo override) e/ou por modificação; a checagem entra no fluxo de transação (falha de validação = status `failed` + rollback, igual a um localizador que não casou). LIMITE A DEIXAR EXPLÍCITO no guia e na mensagem: valida SINTAXE, não SEMÂNTICA — "compila" não é "faz o certo"; o diff e a §8 (verificação pela IA) seguem sendo a rede para o erro lógico. Baixo risco, alto valor percebido, reusa toda a infra de backup/rollback. Pesquisa: `py_compile`/`ast.parse` é a prática padrão de checagem barata; o padrão "validate-then-apply / restore-on-failure" que o ASU já segue só ganharia mais um validador. Candidata forte a uma das próximas features (encaixa em F3, ao lado do "copiar saída" e do gerador de `.bat`).
