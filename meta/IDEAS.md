...
### 2026-06-28 — Atalho .bat "abrir GUI" (clássico, sem args) — SPECADO (meta/specs/F2-bat-fix-e-launcher-classico.md)
Além do .bat por projeto (pré-preenche raiz+instrução), o usuário quer um SEGUNDO botão na GUI que gera um .bat que só ABRE a interface (como abrir o console e rodar `python -m src.gui`), para pôr em qualquer lugar e abrir com 2 cliques mesmo se mover as coisas. Ideia veio do chat do FlatDrop (que gerou `flatdrop-ui.bat`: `start "" pythonw ...\run.py`, sem console). Desenho: função pura `build_open_gui_bat` com `pythonw.exe` + `start "" /d "<asu_home>"` (sem janela de console, diretório de trabalho correto). Atalho independente de projeto — salvar onde quiser (FlatDrop sugeriu pasta `launcher`; opcional).

### 2026-06-28 — BUG do .bat por projeto: `--instruction-dir "%~dp0"` quebra o argumento — SPECADO (correção)
O `abrir-asu-fileview.bat` não abria apontado como esperado. Causa-raiz: `%~dp0` SEMPRE termina em `\`, então `"%~dp0"` vira `"...\"` e a sequência `\"` é lida como aspa escapada (regras de linha de comando do C runtime/Python) → `--instruction-dir` corrompido, resolução pasta→instrução falha. `--root "%~dp0fileview"` não sofre (não termina em `\`). Correção: emitir `--instruction-dir "%~dp0."`. Bônus: `chcp` precisa considerar `bat_dir` (= `%~dp0` em runtime); pasta do .bat acentuada → chcp+UTF-8.

### 2026-06-28 — Local do backup pela GUI + nome por projeto (quando externo) — SPECADO (meta/specs/F3-backup-na-gui.md)
Pedido (260628): (a) gerar o backup FORA do repositório (pasta antes da raiz), e (b) não nascer com nome genérico `backups/` — saber de qual projeto é. Veredito: (a) FAZER — já existe no núcleo (`--backup-dir`/`backup_location`, DEC-018), falta o campo na GUI; (b) FAZER SÓ quando externo — dentro do projeto `backups/` basta e prefixar alongaria caminhos (MAX_PATH, FIX-008); quando externo, aninhar `<backup-dir>/<nome-da-raiz>/<timestamp>/` resolve a mistura com 1 nível (mínimo, melhor que prefixar cada arquivo; o espelho já é raso por FIX-008).
...
### 2026-06-28 — Diretriz «Saída de código via ASU» deve distinguir EDITAR (→ASU) de CRIAR (→baixar) — mensagem preparada
