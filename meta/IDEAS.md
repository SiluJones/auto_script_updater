...
### 2026-06-22 — Pastas-raiz recentes e fixadas na GUI (acesso rápido) — SPECADA na F2 (DEC-022)
O usuário quer praticidade: poder escolher entre pastas-raiz RECENTES, ou deixar FIXADAS (salvas) as que costuma usar, para acesso rápido na interface ("uma página de recentes ou salvas"). Casa com — e expande — o item de ROADMAP "histórico dos últimos 5 raízes" (que era só recência). Desenho fechado na spec `meta/specs/F2-acesso-rapido.md`: menu "Recentes ▾" (até 8) + botão "📌 Fixar" ao lado do campo de raiz, persistido no QSettings (`recent_roots`/`pinned_roots`). Selecionar uma entrada preenche a raiz. Vai junto com os args de lançamento e o gerador de .bat (mesmo increment de "Acesso rápido").
...
### 2026-06-15 — Geração de .bat para automação por projeto — SPECADA na F2 (DEC-022; spec em meta/specs/F2-acesso-rapido.md)
O usuário quer .bat por pasta de projeto para acelerar a abertura da GUI já apontada. EVOLUÇÃO 2026-06-22 (decisões fechadas, ver DEC-022): (a) o .bat NÃO auto-aplica — só PRÉ-PREENCHE a raiz e a pasta da instrução; o dry-run e a revisão continuam manuais (o usuário foi explícito: "não quero que pule o dry e eu checar"); (b) o .bat chama `.venv\Scripts\python.exe -m src.gui` DIRETO, sem `activate` (mais robusto — pesquisa de launcher Windows); (c) ele passa uma PASTA de instrução (`--instruction-dir`), não um arquivo, porque os nomes das instruções mudam — a GUI resolve pasta→arquivo escaneando só o topo (1 yaml = pré-preenche; 2+ = abre o seletor na pasta; arquivadas em subpastas são ignoradas), o que resolve o "perigo de vários yaml"; (d) o botão "Criar atalho .bat…" fica na GUI e salva por padrão na pasta-pai da raiz. A pesquisa anterior (AI-patch safety: auto-aplicar sem revisão é arriscado) está honrada justamente por (a). PESQUISA de launcher: "se o projeto depende de libs do venv, não use activate.bat; aponte direto para o python.exe do venv" (Python docs + comunidade). Resta só implementar (Claude Code, via a spec).
