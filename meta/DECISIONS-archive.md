# DECISIONS-archive.md — Decisões Arquivadas (F0–F1)

> Arquivo-baú. Recebeu as entradas mais antigas do `DECISIONS.md` quando ele passou de ~700 linhas (gatilho do próprio arquivo).
> Contém as decisões fundacionais (stack, arquitetura, schema) e os bugs de intake iniciais — período 2026-06-03 a 2026-06-10 (F0 + F1).
> Consultado sob demanda; NÃO é lido no ritual de início. As entradas ativas seguem em `DECISIONS.md`.
> Numeração preservada: DEC-001..DEC-012 e FIX-001..FIX-006. Não renumerar — referências cruzadas apontam por ID.
